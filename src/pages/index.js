...
export default function Home() {
  ...
  const {isLoaded} = useJsApiLoader({googleMapsApiKey: 'AIzaSyBHJxzPHD_egYnhxntqcvfem35YRjruzAg', libraries: ['places']})
    ...
    <Tab.Group onChange={(index) => TabChange(index)}>
      ...
      <Tab.Panels>
        ...
        <Tab.Panel className='p-4'>
          <div className='flex flex-col'>
            <p className='text-3xl font-bold'>Request a ride now</p>
            <GoogleMap onLoad={(map) => setMap(map)} center={MapCenter} zoom={15} mapContainerStyle={{width: '400px', height: '400px'}} options={{zoomControl: false, streetViewControl: false, mapTypeControl: false, fullscreenControl: false}}>
            ...
            <div className='flex flex-col'>
              <button onClick={() => map.panTo(MapCenter)} className='h-10 justify-center font-semibold text-white rounded-md bg-black px-4' type='submit'>Request now</button>
              <button className='h-10 justify-center font-semibold text-black rounded-md px-4' type='submit'>Schedule for later</button>
            </div>
          </div>
        </Tab.Panel>
