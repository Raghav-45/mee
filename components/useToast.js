...
export const UseMyToast = (props) => {
  ...
  // // useEffect(() => {
  // //   let timerOut = setTimeout(() => {setShowToast(false)}, 1000)
  // //   return () => {clearTimeout(timerOut); setShowToast(false)}
  // // }, [])
  setTimeout(() => {setShowToast(false)}, 5000)

  return (

    // <div dangerouslySetInnerHTML={{__html: '<p>raghav</p>'}}></div>

    <div id="toast-warning" className={`flex items-center w-full max-w-xs p-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 ${showToast && 'showToast'}`} role="alert">
