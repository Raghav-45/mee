...
export const UseMyToast = (props) => {
  // const [showToast, setShowToast] = useState(true)
  const [showToast, setShowToast] = useState(true)
  const [toastList, setToastList] = useState(['e','f','g','h'])
  ...
  useEffect(() => {
    if (['success', 'warning'].includes(props.ToastType)) {
      setToastList([...toastList, props.ToastType])
    }
  }, [props.ToastType])
  ...
  return (
    toastList.map((elem) => <p>{elem}</p>)
  )

  // if ( showToast ) {
  //   return (
  //     <div id="toast-warning" className={`flex items-center w-full max-w-xs p-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 showToast`} role="alert">
...
export const ShowToast = () => {
  return (
    <UseMyToast show={true} time={5000} ToastType={'success'} />
  )
}